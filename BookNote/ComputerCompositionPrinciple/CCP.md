...
- 存储速度一般使用数据传输率来衡量的，而数据传输率与数据的宽度和存储周期有关
...
- 上图中，译码器具有多个使能，$G_1$ 高电平有效， $\overline{G_{2A}}$ 、 $\overline{G_{2B}}$ 低电平有效，当三个同时有效时，译码器工作。
- 因此，如果我们将CPU上的存储器读写请求 ( $\overline{MREQ}$ ) 绑定到 $\overline{G_{2B}}$ 后，当CPU传输地址到译码器稳定后，再输送存储器请求信号，使译码器开始工作，这样有利于数据的稳定

### 双端口RAM

> 在很多时候，CPU的读写速度是远高于RAM的，而且现代计算机也不只一个CPU在运作，因此，寻求一种合适的方式来提高读写效率是有必要的，因此就有了双端口RAM

- 双端口RAM顾名思义就是有两个接口的RAM，其能够同时接收两套从CPU发送数据，能够并行处理，用于优化多核CPU访问一根内存条的速度

![[双端口RAM.png]]

- 双端口RAM要求了两组完全独立的数据线、地址线、控制线。CPU、RAM中也要有更复杂的控制电路
- 而两个端口对同一主存操作则有以下四种情况
	- 1. 两个端口同时对**不同的地址单元存取数据**
	- 2. 两个端口同时对**同一地址单元读出数据**
	- 3. 两个端口同时对**同一地址单元写入数据**
		- 写入错误
	- 4. 两个端口同时对**同一地址单元，一个写入数据，另一个读出数据**
		- 读出错误
- 为了防止数据冲突，CPU中设置了**BUSY**信号，由判断逻辑决定暂时关闭一个端口(延时操作)，未被关闭的端口正常访问，被关闭的端口延长一个很短的时间段后再访问

### 多模块存储器

> **多模块存储器是一种空间并行技术，利用多个结构完全相同的存储器模块的并行工作来提高存储器的吞吐率**。常用的有单体多字存储器和多体低位交叉存储器

#### 单体多字存储器

> 单体多字系统的特点是**存储器中只有一个存储体，每个存储单元存储m个字，总线宽度也是m个字。一次并行读出m个字，地址必须顺序排列并处于同一存储单元**

![[单体多字存储器.png]]

- 单体多字系统在一个存储周期内，从同一地址取出m条指令，然后将指令逐条送往CPU执行，即每隔 $\frac{1}{m}$ 存储周期，CPU向主存取一条指令，显著提高了单体存储器的工作速度
- **但是，指令和数据在主存中必须是连续存放的，一旦遇到转跳指令，或操作数不能连续存放，这种方法的效果反而不明显**

#### 多体并行存储器

> 多体并行存储器由多体模块组成。每个模块都有相同的容量和存取速度，各模块都有独立的读写控制电路、地址寄存器和数据寄存器。既能并行工作，又能交叉工作
...
- 高位交叉方式下，总是把低位的体内地址送到由高位地址体号确定的模块内进行译码。**访问一个连续主存块时，总是先在一个模块内访问，等到该模块访问完才转到下一个模块访问，CPU总是按顺序访问存储模块，各模块并不能并行访问，因此不能提高存储器的吞吐率**
- 因此，**高位交叉编址的方式，只能对存储容量进行扩容，而对效率的提高影响不大**
...
- 低位交叉方式下，总是把高位的体内地址送到由低位体号确定的模块内进行译码。**程序连续存放在相邻模块中，因此采用此方法为交叉存储器。采用低位交叉编址后，可在不更改每个模块存取周期的前提下，采用流水线的方式并行存取，提高存储器的带宽**

![[高-低位交叉编址.png]]

- 设模块字长等于数据总线宽度，模块存取一个字的存取周期为 $T$，总线传送周期为 $r$，为实际流水线方式存取，则存储器交叉模块数应该大于等于

$$
	m = \frac{T}{r}
$$
- 式中，**m称为交叉存取度。每经过r时间延迟后启动下一模块，交叉存储器要求其模块数必须大于等于m，以保证启动某模块后经过 $m \times r$ 的时间后再次启动该模块时，其上次的存取操作已经完成(即流水线不间断)。**
- 这样，连续存取m个字所需的时间为
...
- 常见的两种描述
	- 存取周期为 $T$， **存取时间为r**，为了使流水线不间断，应保证模块数 $m \ge T/r$ 
	- 存取周期为 $T$，**总线传输周期为r**，为了使流水线不间断，应保证模块数 $m \ge T/r$ 
